module pingpong_buffer #(
    ...
    //============================================================
    // 3) 写域 -> 读域：commit toggle（每个bank一个）
    //    含义：该bank有一帧完整数据提交了
    //============================================================
    ...
    //============================================================
    // 4) 读域 -> 写域：release toggle（每个bank一个）
    //    含义：该bank已经读完，可以重新FREE
    //============================================================
    ...
    //============================================================
    // 5) frame_start_addr 跨域同步
    //    约束：某bank提交后，其对应start_addr保持不变直到该bank释放
    //============================================================
    ...
    //============================================================
    // 6) 读域内部：active / pending
    //    pingpong最多只会挂一个pending
    //============================================================
    reg              active_bank_rd;
    reg              pending_valid;
    reg              pending_bank_rd;
    ...
    // 8) 写域主逻辑
    //
    // 上游sample_controller约定：
    // capture_done 与最后一个 we 同拍出现。
    // 因此本模块在 capture_done 当拍：
    // 1. 先写入最后一个样本
    // 2. 再将当前写bank提交为READY
    //============================================================
    always @(posedge clk_write or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // 正常写入
            if (we && curr_bank_is_writing) begin
            ...
            // 之前overflow了，当前没有bank处于WRITING
            // 这时只要有FREE bank出现，就恢复写入
    ...
    //============================================================
    // 9) 读域：同步commit toggle与每个bank的start_addr
    //============================================================
